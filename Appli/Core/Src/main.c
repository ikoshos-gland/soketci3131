...
int main(void)
{
  ...
  if ( DSP_Pipeline_Init () != HAL_OK ) {
	  Error_Handler () ;
  }

  if ( ADS1299_Init (& hspi1 ) != HAL_OK ) {
	  Error_Handler () ;
  }
...
static void MX_GPIO_Init(void)
{
  ...
  /* RESET Pin (Output, High by default) */
  ...
  /* START Pin (Output, Low by default) */
